...
<script>
    ...
    cargarGrilla = function(respuesta,total) {
        $('#spinner').show();
        $.post("<?php echo url_for('inscripAsignatura/traeInscripAsignatura') ?>",
        { carre: $('#carre').val()}, 
            function(data){
...
<form id="examenesForm" class="" name="examenesForm" method="POST" action="<?php echo url_for("inscripAsignatura/Anotar") ?>" >

<?php 
	$cabecera = new Cabecera();
	$cabecera->ruta('Incripción de asignaturas'); 
...
?>
    ...
    <br>
     ...
     <!--   <input type="hidden" id="classEnti" value="<?php echo $codigo_prov ?>">  -->
